...
| OPT-009 | 商家分配系统改造 | ✅ 已开发 | 新增 merchant_account_relationships 表+AES-256 token 加密+7 平台 API 同步服务+佣金拆分(自跑/分配)+MID 行内补录+前端 Tag 平台分布+申请状态列+佣金明细弹窗+待补 MID Tab+scheduler 16:00 同步+Alembic 009 迁移 |
...
| 文件路径 | 说明 | 状态 |
|----------|------|------|
| `backend/app/models/merchant_account_relationship.py` | 商家-账号申请关系表 | ✅ 已开发 |
| `backend/app/utils/crypto.py` | AES-256 token 加解密工具 | ✅ 已开发 |
| `backend/app/services/merchant_platform_sync.py` | 平台商家同步服务 + 7 平台适配器 | ✅ 已开发 |
...
| `backend/app/models/affiliate_account.py` | 新增 api_token_encrypted、token_updated_at 字段（OPT-009） | ✅ 已开发 |
| `backend/app/models/merchant.py` | 新增 relationship_status 字段（OPT-009） | ✅ 已开发 |
| `backend/app/services/merchant_service.py` | 新增 get_commission_breakdown() 佣金拆分方法（OPT-009） | ✅ 已开发 |
| `backend/app/api/merchants.py` | 新增同步触发端点 + 佣金明细端点 + 列表返回新字段（OPT-009） | ✅ 已开发 |
| `frontend/src/pages/MerchantManagement.jsx` | 平台分布优化、表格列扩展、MID 编辑、佣金弹窗、待补 MID Tab（OPT-009） | ✅ 已开发 |
...
| 方案设计（OPT-009 商家分配系统改造） | AI 审核官 | ✅ 设计完成，代码已开发 | 迭代 1.8 新增，已补齐第十一轮口径修正。代码已全部落地：3 个新建文件 + 5 个后端修改 + 1 个前端改造 + Alembic 009 迁移 + scheduler 16:00 任务 |
