...
> 🔧 **迭代 1.2 确认：** 使用独立服务账号，新增配置键名。

**新增配置项（`backend/app/config.py`）：**

```python
# Google Sheets 同步 - 独立服务账号
google_sheets_service_account_file: str = ""
google_sheets_service_account_json_base64: str = ""
```

- 与 Google Ads 服务账号分开管理，互不影响
- 优先使用 `google_sheets_service_account_json_base64`（适合服务器部署）
- 需在 Google Cloud Console 中为该服务账号启用 Google Sheets API
- Sheets API 范围：`https://www.googleapis.com/auth/spreadsheets.readonly`
- 需将服务账号邮箱添加为目标 Sheet 的「查看者」
...
function main() {
  ...
  var accountIterator = AdsManagerApp.accounts()
    .withCondition("manager_customer.status = ENABLED")
    .get();
...
| `backend/app/config.py` | 新增 `REJECTED_COMMISSION_DAILY_THRESHOLD` + `google_sheets_service_account_*` 配置项 |
