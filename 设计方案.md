...
| `backend/app/api/bid_management.py` | 新增 /api/campaigns/toggle-status 和 /api/campaigns/batch-toggle-status 接口（OPT-008） |
| `backend/app/services/google_ads_service_account_sync.py` | 新增 toggle_campaign_status 方法（OPT-008） |
| `backend/app/api/google_ads_aggregate.py` | by-campaign 接口返回追加 mcc_id、sync_mode、customer_id 字段（OPT-008） |
...
| `frontend/src/pages/DataCenter.jsx` | 待实现项：OPT-003 已付佣金展示 + OPT-004 ROI 公式修改 + OPT-008 广告系列暂停/启用操作列与批量选择 |
...
| 方案设计（OPT-008 广告暂停/启用） | AI 审核官 | ⏳ 待审核 | 迭代 1.7 新增，复用已有 mutate 基础设施，需确认权限隔离和脚本模式限制 |
...
经对 17、18 章交叉复核，方案主体正确、架构方向可落地，但存在 4 类高频审核追问风险：
...
| GAP-03 | 迁移路径 | 缺少 P0 多表迁移拆分执行顺序 | 高 | 新增 19.4 迁移分批方案 | 否 |
...
3. 若审核通过，建议按 19.4 的 Batch-1 → Batch-2 → Batch-3 顺序执行，避免一次性大迁移风险。
...
| 2026-03-03 | 07（验收） | 章节编号修复（M-16） | 修复设计方案内十七、十八、十九章子标题编号与章标题不一致 | 统一编号后内部交叉引用全部更新（CR-015） |

---

> 本节作为「网站功能优化1.0」迭代 1.7 提审增补：新增 OPT-008 广告系列暂停/启用功能设计，修复 M-16 章节编号错位，不改变既有已上线功能边界。
