...
import { sqlTxnMonth, sqlTxnRange, REPORT_PLATFORM_ORDER, type MonthData } from "@/lib/report-metrics";
 ...
 * 时区/切日口径统一走 report-metrics（按各平台后台显示时间归月，与平台后台一致）。
 * 返回：
 *   - members[]：团队成员列表
 *   - platforms[]：参与统计的平台列表
...
export const GET = withLeader(async (req: NextRequest, { user }) => {
  ...
  const commRows = await prisma.$queryRawUnsafe<{
    ...
    ORDER BY t.user_id, month, t.platform
