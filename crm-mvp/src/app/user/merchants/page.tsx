...
interface Merchant {
  ...
  // D-338：LB 待审核佣金（全员维度）。非 LB 商家为 null
...
export default function MerchantsPage() {
  ...
  // C-112 / D-046.A 1.B 已下架：员工不再手动维护商家画像
  // 画像由 ad-creation 流程在生成广告时由 AI 自动生成（IntelliCenter 8 步闭环 Step 3）
  // 人设库管理
...
// __COLUMNS__
  ...
  // D-338：LB 待审核佣金列渲染。仅 LB 商家有值（后端非 LB 返回 null → 显示「-」）。
  ...
  const availCols = useMemo(() => [
    ...
    { title: "拒付率", dataIndex: "chargeback_rate", width: 76, align: "center" as const, render: renderChargebackRateCol },
    { title: "结算率", dataIndex: "settlement_rate", width: 76, align: "center" as const, sorter: true, sortOrder: colSortOrder("settlement_rate"), render: renderSettlementRateCol },
    // D-338：LB 待审核佣金，与"我的商家"同列同口径（领取前就要看见）
    { title: "待审核佣金", dataIndex: "lb_pending_commission", width: 100, align: "right" as const, render: renderLbPendingCol },
    { title: "支持地区", dataIndex: "supported_regions", width: 116, render: (v: unknown[] | null) => <RB r={v} /> },
  ...
  return (<div style={{ maxWidth: 1600, margin: "0 auto" }}>
    ...
    <Card className="merchant-table-card">
      ...
      {tab === "chargebacks" && (<div>
        <div style={{ marginBottom: 12, padding: "8px 12px", background: "#fffbe6", border: "1px solid #ffe58f", borderRadius: 6, fontSize: 12, color: "#666" }}>
          ...
          全员拒付率聚合数据（时间窗默认 2025-11-01 至今）。分母基于全部订单佣金总额（含待结算）。
        </div>
                    ...
                    render: (id: string) => {
                      ...
                      const tip = (() => {
                        if (details.length > 0) {
                          return (
                            <div style={{ fontSize: 12, lineHeight: 1.6 }}>
                              <div>近 7 天在投 {c.ad_count >= 2000 ? "≥2000" : c.ad_count} 条；OCR 识别 {resolvedCount} 条 → {details.length} 个域名（重复率 {dupPct}%）：</div>
                      ...
                      if (c.classification === "peer") {
                        return (
                          <Tooltip title={tip}>
                            <Tag color="green">同行 · 投{c.ad_count >= 2000 ? "≥2000" : c.ad_count}条 · {c.unique_domain_count}域名</Tag>
                          </Tooltip>
                        );
                      }
    ...
    {/* C-112 / D-046.A 1.B 已下架：员工不再手动维护商家画像，由 ad-creation 自动生成 */}
  </div>);
}
