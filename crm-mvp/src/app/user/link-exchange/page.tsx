...
export default function LinkExchangePage() {
  ...
  const linkColumns: ColumnsType<CampaignRow> = [
    ...
    {
      title: <Tooltip title="今日(北京时间)该商家的联盟平台点击数。RW（Rewardoo）为聚合联盟、点击接口对我方恒返回 0，故 RW 改用 Google 侧真实点击(ads_daily_stats)作为点击口径">点击数</Tooltip>, width: 90, align: "center",
      render: (_: unknown, row) =>
        row.platform === "RW" ? (
          <Tooltip title="RW 点击接口对我方恒返回 0，此处显示 Google Ads 真实点击（同一条 RW 跟链上，Google 点击≈联盟可见点击）">
            <Space size={2}>
              <Text style={{ fontSize: 12 }}>{row.todayClicks}</Text>
              <Tag color="blue" style={{ margin: 0, fontSize: 10, lineHeight: "14px", padding: "0 3px" }}>G</Tag>
            </Space>
          </Tooltip>
        ) : (
          <Text style={{ fontSize: 12 }}>{row.todayClicks}</Text>
  ...
  const ALERT_GUIDE: Record<string, string[]> = {
    ...
    no_tracking_stuck: [
      ...
      "② 与「链接无效」的区别：那个是根本跟不到商家；这个是跟到了但点击不算你的。",
      "③ 报到你面前之前系统已经换了另一套跟跳引擎复验过一遍（能执行 JS 跳转、带 Cookie 和来路），仍然拿不到参数。所以基本可以排除「是我们跟不动」，等系统自愈无用。",
    ...
    connection_mismatch: [
      ...
      "④ 跨平台的（如名字写 CG1、却归属在 MUI1）也点「纠正归属」：商家关联会一并换成 CG 平台下的那条，链接通常当场就有。系统会先核对这个 MID 在 CG 平台下确实存在，不存在才会拦你。",
