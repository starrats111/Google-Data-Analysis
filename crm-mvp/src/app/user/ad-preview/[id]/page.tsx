...
function CoreGeneratingCard({ active }: { active: boolean }) {
  ...
  const STAGES: Array<{ from: number; title: string; hint: string; color: string }> = [
    { from: 0,  title: "正在分析商家网站",       hint: "读取页面内容、识别品牌定位与核心卖点…",                          color: "#1677ff" },
    { from: 30, title: "正在生成标题与描述",     hint: "AI 正在撰写广告标题与描述，通常需 15–40 秒…",                      color: "#13c2c2" },
    { from: 60, title: "正在抓取站内链接与图片", hint: "补充网站上下文，抓取真实子页面与产品图，请稍候…",                  color: "#722ed1" },
    { from: 90, title: "深度探查中",             hint: "部分网站反爬较严，正用真人浏览器模式深探，可能需 2–3 分钟，您可先操作其它字段。", color: "#fa8c16" },
  ];
  let stageIdx = 0;
  ...
  const fetchKeywordsFromSemrush = useCallback(async (opts?: { manual?: boolean }) => {
    ...
    try {
      ...
      if (newKws.length > 0) {
        ...
        const organicCount = newKws.filter((k) => k.source === "semrush_organic").length;
        const parts: string[] = [];
        if (paidCount > 0) parts.push(`${paidCount} 个付费词`);
        if (organicCount > 0) parts.push(`${organicCount} 个自然词`);
        const baseMsg = `AI 已优选 ${newKws.length} 个关键词（${parts.join(" + ")}）`;
      ...
      <Card size="small" style={{ marginBottom: 12 }} styles={{ body: { padding: "10px 20px" } }}>
        <Steps
          ...
          items={[
            {
              title: "关键词（AI 优选）",
              description: kwList.length > 0 ? `AI 已优选 ${kwList.length} 个，可调整` : "AI 正在从自然词+付费词中优选…",
            },
            {
              title: "文案与扩展",
                ...
                <div key={`${kw.text}-${kw.matchType}-${i}`} style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "8px 10px", border: "1px solid #d9e8ff", borderRadius: 8, background: kw.score != null && kw.score >= 70 ? "#f6ffed" : "#f8fbff" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center", marginBottom: 4 }}>
                      ...
                      {kw.source === "semrush_paid" && <Tag color="volcano" style={{ margin: 0, fontSize: 11 }}>AI优选·付费词</Tag>}
                      {kw.source === "semrush_organic" && <Tag color="cyan" style={{ margin: 0, fontSize: 11 }}>AI优选·自然词</Tag>}
          ...
          {currentStep === 0 && (
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
                <div>
                  <Text strong style={{ color: "#0958d9", fontSize: 14 }}>关键词（AI 已自动优选，可调整）</Text>
                  <Text type="secondary" style={{ display: "block", fontSize: 12, marginTop: 2 }}>
                    AI 已从「自然词 + 付费词」中优选 <Text strong>{kwList.length}</Text> 个，并自动开始生成。如需手动增删关键词，调整后点此重新生成
                  </Text>
                </div>
                <Button
                  type="primary"
