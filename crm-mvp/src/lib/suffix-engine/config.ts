...
export const STOCK_CONFIG = {
   ...
   *  2026-07-24 降耗：60→90 分钟（kookeey 流量真实耗尽事故后温和降频约 1/3；目标水位 5/低水位 2 不变，
   *  不伤成货率——消耗大头是浏览器整页流量，补货轮次直接决定浏览器启动次数）。 */
  BROWSER_REPLENISH_COOLDOWN_MS: 90 * 60_000,
  ...
  CONCURRENCY: 5,
  /** D-231：本轮 probe 用上了无头浏览器的系列，批量阶段改用这个更低的并发。
   *  换链接能同时跑的浏览器实际只有 exchange 车道那几个槽（保底 1 + 借来的余量，总池封顶 3），
   *  5 路齐发只会让多出来的 3 路在队列里干等 30s 再超时——既不产出，还把队列堆给刷点击。
   *  取 2：贴合「快车道 1 + 弹性/借预留通常还能拿到 1」的真实可用量，满了就等，不再过量投喂。 */
  ...
  /** D-351 连续多少轮「本机资源开不出浏览器」（local_resource）后，连 force 路径也不再放行。
   *  取 3 与 D-201 对齐：真的只是瞬时内存尖峰时，10min 冷却到期重试两次足够跨过去；
   *  连续 3 轮还开不出来说明是持续性资源紧张，再打下去只会加深紧张（每次重试都先跑完 HTTP 抓取）。
   *  人工入口（manual）永远穿透——人在页面上补货时要能立刻试。 */
  LOCAL_BLOCK_STREAK_THRESHOLD: 3,
  /** D-351 达阈值后的冷却：比 10min 长得多，但远短于死链的 8h——资源紧张会自己缓解，
   *  不该像坏链那样锁半天。取 1h：既把重试从「每分钟 5 次」压到「每小时 1 次」，
   *  又保证夜间高峰过去后当轮就能自动恢复补货。 */
  LOCAL_BLOCK_COOLDOWN_MS: 60 * 60_000,
