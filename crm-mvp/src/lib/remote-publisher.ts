...
// ─── sitemap.xml 同步 ───────────────────────────────────────────────────────
//
// 调查结论（2026-09-19，48 个站全量盘点）：只有 4 个站有 sitemap.xml，其余 44 个
// 没有；而那 4 个里 cavendishrow / nexolume 由本地 build.py 生成，每次重建都整体
// 重写 sitemap.xml，publisher 写进去的条目会被下一次部署抹掉。剩下两个
// （sipsavorly.top / all-set-dining.top）的 <loc> 和 robots.txt 都指向套模板留下
// 的死域名（sipsavorly.com / allsetdining.com 均不解析），继承它们的内容只会得到
// 一份跨域名的垃圾 sitemap —— Google 会直接忽略这种条目。
//
// 所以这里不解析、不继承任何现有内容：CRM 只维护自己那份清单，用站点的真实域名
// 重新生成。生成器站靠哨兵注释识别后跳过（见 SITEMAP_EXTERNAL_MARKERS）。

const SITEMAP_MARKER = "crm-managed-sitemap";

// 本地生成器写出的 sitemap 里会带这些标记之一；命中则说明该站的 sitemap 由站点
// 自己的构建脚本管理，CRM 不能碰（写了也会被下一次重建覆盖）。
const SITEMAP_EXTERNAL_MARKERS = ["changefreq", "<image:", "generated by"];
...
/** CRM 维护的 sitemap 条目：{loc, lastmod} */
type SitemapEntry = { loc: string; lastmod: string };

function buildCrmSitemap(entries: SitemapEntry[]): string {
  const body = entries
    .map(
      (e) =>
        "  <url>\n" +
  ...
  return (
    ...
    `<!-- ${SITEMAP_MARKER}: 由广告自动化平台维护，仅含 CRM 发布的文章。` +
    "站点自有页面靠内链抓取。手工编辑会在下次发布时被覆盖。 -->\n" +
...
/** 从 CRM 自己生成的 sitemap 里读回条目；不是 CRM 生成的则返回 null。 */
function parseCrmSitemap(xml: string): SitemapEntry[] | null {
  if (!xml.includes(SITEMAP_MARKER)) return null;
  ...
  while ((m = re.exec(xml)) !== null) {
    ...
    if (!loc) continue;
    const lastmod = /<lastmod>([\s\S]*?)<\/lastmod>/.exec(m[1]);
    out.push({
      loc: loc[1].trim(),
...
/**
 * 把一条文章写入/移出站点的 sitemap.xml。
 *
 * 非致命：任何失败都只记日志，不影响发布本身（与 syncToHomepageDataFile 一致）。
 *
 * @param remove true 表示移除该 loc（撤回文章时用）
 */
async function syncSitemapEntry(
  sftp: SFTPWrapper,
  siteRoot: string,
  domain: string,
  detailUrl: string,
  ...
  try {
    if (!domain || !detailUrl) return;
    const sitemapPath = `${siteRoot}/sitemap.xml`;

    // 目录模式（post-slug）对外是带尾斜杠的 clean URL；带 ? 的查询式详情页不进
    // sitemap —— 同一个 article.html 反复出现只会稀释抓取预算。
    if (detailUrl.includes("?")) return;
    const loc = `https://${domain}/${detailUrl.replace(/^\/+/, "").replace(/\/+$/, "")}/`;

    let existing = "";
    try {
      existing = await sftpReadFile(sftp, sitemapPath);
    } catch {
      existing = "";
    }

    if (existing) {
      const mine = parseCrmSitemap(existing);
      if (mine === null) {
        // 别人的 sitemap。生成器写的（带 changefreq / image 命名空间等）绝不能碰：
        // 下次重建会覆盖，白写。其余情况同样保守跳过 —— 宁可没有条目，也不要把
        // 一份手工维护的 sitemap 改坏。
        const low = existing.toLowerCase();
        const why = SITEMAP_EXTERNAL_MARKERS.find((k) => low.includes(k));
        console.log(
          `[Publisher] ${domain} 的 sitemap.xml 非 CRM 生成（${why || "无 CRM 标记"}），跳过`,
        );
        return;
      }
      const kept = mine.filter((e) => e.loc !== loc);
      if (remove) {
        if (kept.length === mine.length) return; // 本来就不在里面
        await sftpWriteFile(sftp, sitemapPath, buildCrmSitemap(kept));
        return;
      }
      kept.unshift({ loc, lastmod });
      await sftpWriteFile(sftp, sitemapPath, buildCrmSitemap(kept));
      return;
    }

    // 没有 sitemap.xml —— 44 个站属于这种情况。撤回时无事可做。
    if (remove) return;
    await sftpWriteFile(sftp, sitemapPath, buildCrmSitemap([{ loc, lastmod }]));
    console.log(`[Publisher] ${domain} 新建 sitemap.xml`);
  } catch (err) {
    console.warn(`[Publisher] 同步 sitemap.xml 时出错（非致命）:`, err);
  }
}
...
export async function publishArticleToSite(
  ...
  try {
    ...
    // 5b. sitemap.xml —— 没有这一步，新文章只有拿到确切 URL 才能被访问到，
    // Google 无从发现（站点自有页面有内链，CRM 文章没有）。
    await syncSitemapEntry(sftp, siteRoot, site.domain, detailUrl, dateISO);
...
export async function unpublishArticleFromSite(
  ...
  try {
    ...
    if (slug) {
      ...
      // 4. 从 sitemap.xml 移除 —— 漏掉这步会让 sitemap 累积死链，
      // 比没有 sitemap 更糟。
      await syncSitemapEntry(sftp, siteRoot, site.domain, detailUrl, "", true);
    }
...
export async function updateArticleDateOnSite(
  ...
  try {
    ...
    const varName = site.article_var_name || "articlesIndex";
    const fullDataPath = `${siteRoot}/${dataJsPath}`;
    // sitemap 的 <lastmod> 要跟着改，否则显示的还是首发日期。detailUrl 只有索引
    // 条目里有（本函数不收 slug 参数），所以在这里顺手取出。
    let detailUrlForSitemap = "";
    try {
      ...
      if (existing) {
        ...
        if (idx >= 0) {
          ...
          detailUrlForSitemap = typeof prev.detailUrl === "string"
            ? prev.detailUrl
            : (typeof prev.slug === "string" && prev.slug ? `post-${prev.slug}` : "");
          existing[idx] = { ...prev, date: dateLabel, dateISO: publishDate.toISOString().slice(0, 10), dateLabel };
    ...
    // 4. 同步 sitemap 的 <lastmod>
    if (detailUrlForSitemap) {
      await syncSitemapEntry(
        sftp, siteRoot, site.domain, detailUrlForSitemap,
