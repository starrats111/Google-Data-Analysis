...
export async function crawlWithPuppeteerFull(
  ...
  try {
    ...
    // D-088 回滚 D-085：恢复"提取前快速丢弃"门槛。D-085 把丢弃后移到 DOM 提取之后，
    // 导致硬反爬站（如 qronge.com）即便是 CF 挑战墙也被保留，继续跑下游慢速站内链接
    // Puppeteer 兜底验证 → buildCrawlCache 卡满 300s、生成槽位被长期占用、排队请求 180s 超时
    // → 商家信息根本出不来。恢复旧 fail-fast：看着被拦截/内容过小直接 return null，
    // 让生成走降级路径快速产出文案。
    if (isBlockedPage(html) || html.length < 3000) {
      console.log(`[Crawler] Puppeteer: 仍被拦截或内容过小 (${html.length} bytes)`);
      return null;
    }
    ...
    // D-088 回滚 D-085：移除后移的"真·拦截墙"二次判定（已在提取前 fail-fast）。
