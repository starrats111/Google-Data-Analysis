...
# API 配置
DEFAULT_BASE_URL = "https://www.linkbux.com"
TRANSACTION_API = f"{DEFAULT_BASE_URL}/api.php?mod=medium&op=transaction_v2"


class LinkBuxService:
    """
    LinkBux API 服务类
    
    API文档要点：
    - 端点: GET https://www.linkbux.com/api.php?mod=medium&op=transaction_v2
    ...
    def __init__(self, token: str):
        ...
        Args:
            token: LinkBux API token
        """
        self.token = token
        logger.info(f"[LB Service] 初始化，API={TRANSACTION_API}")
    ...
    def sync_transactions(
        ...
        while True:
            ...
            page += 1
            if page > max_pages:
                logger.warning(f"[LB API] 分页超过最大页数 {max_pages}，停止获取")
                break
        
        logger.info(f"[LB API] 共获取 {len(all_transactions)} 条交易记录")
    ...
    def _get_transactions_paginated(
        ...
        try:
            logger.info(f"[LB API] 请求交易数据: {begin_date} ~ {end_date}, page={page}, limit={limit}")
            ...
            if code != 0:
                ...
                if code in error_code_map:
                    ...
                    logger.error(f"[LB API] 请求失败: code={code}, msg={error_msg}")
                    return {
                        "success": False,
            ...
            logger.info(f"[LB API] 返回 {len(transactions)} 条交易记录 (page {page})")
    ...
    def extract_transaction_data(self, result: Dict) -> List[Dict]:
        ...
        for item in transaction_list:
            ...
            # 提取商家ID（MID）：LB 优先 merchant_id，再回退 mid
            mid_value = item.get("merchant_id") or item.get("mid")
