...
# API 配置
DEFAULT_BASE_URL = "https://api.partnermatic.com"
TRANSACTION_API = f"{DEFAULT_BASE_URL}/api/transaction"


class PartnerMaticService:
    """
    PartnerMatic API 服务类
    
    API文档要点：
    - 端点: POST https://api.partnermatic.com/api/transaction
    - 必需参数: source="partnermatic", token, beginDate, endDate
    ...
    def __init__(self, token: str):
        ...
        Args:
            token: PartnerMatic API token
        """
        self.token = token
        logger.info(f"[PM Service] 初始化，API={TRANSACTION_API}")
    ...
    def sync_transactions(
        ...
        while True:
            ...
            page += 1
            if page > max_pages:
                logger.warning(f"[PM API] 分页超过最大页数 {max_pages}，停止获取")
                break
        
        logger.info(f"[PM API] 共获取 {len(all_transactions)} 条交易记录")
    ...
    def _get_transactions_paginated(
        ...
        try:
            logger.info(f"[PM API] 请求交易数据: {begin_date} ~ {end_date}, page={page}, per_page={per_page}")
            ...
            logger.info(f"[PM API] 返回 {len(transactions)} 条交易记录 (page {page})")
    ...
    def extract_transaction_data(self, result: Dict) -> List[Dict]:
        ...
        for item in transaction_list:
            ...
            commission_amount = _safe_float(
                item.get("saleComm") or item.get("sale_comm") or 
