"""Deploy image fix + restart backend"""
...
print("\n=== Verify deployed code ===")
run(f"grep 'min_width=300' {BE_DIR}/app/api/article_gen.py | head -3")
run(f"grep 'crawled_count == 0' {BE_DIR}/app/api/article_gen.py | head -3")
run(f"grep 'testimonial' {BE_DIR}/app/services/merchant_crawler.py | head -3")
