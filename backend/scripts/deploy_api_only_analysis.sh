...
# 1. 拉取最新代码（如果可能）
echo "1. 尝试拉取最新代码..."
git pull origin main 2>/dev/null || echo "   ⚠ git pull 失败，将使用本地代码或手动创建文件"
echo ""
...
# 3. 检查新服务文件是否存在，如果不存在则创建
if [ ! -f "app/services/api_only_analysis_service.py" ]; then
    echo "2. 创建新的分析服务文件..."
...
# 4. 修复 analysis.py API 文件
echo "3. 修复 analysis.py API 文件..."
python3 << 'EOFPYTHON'
import re
...
# 1. 添加导入
if 'from app.services.api_only_analysis_service import ApiOnlyAnalysisService' not in content:
    # 在 AnalysisService 导入后添加
...
# 2. 替换 /process 端点
old_process_pattern = r'@router\.post\("/process"\).*?return response_data'
