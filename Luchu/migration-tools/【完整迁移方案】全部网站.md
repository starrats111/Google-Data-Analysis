# 📦 博客网站数据迁移方案（全部9个网站）

> **创建日期**: 2026-02-13  
> **负责人**: wj07  
> **预计总时间**: 每个网站1-2小时

---

## 📋 迁移概览

| 成员 | 网站名称 | 域名 | 仓库地址 | 数据文件位置 | 产品模块 | 特殊处理 |
|------|----------|------|----------|--------------|----------|----------|
| wj01 | EverydayHaven | everydayhaven.top | starrats111/EverydayHaven | `js/data.js` | ❌ 无 | 标准迁移 |
| wj02 | Quiblo | quiblo.top | starrats111/Quiblo | `data.js`（根目录） | ❌ 无 | ⚠️ 需先移动文件 |
| wj03 | Kivanta | kivanta.top | starrats111/kivanta | `js/data.js` | ❌ 无 | 标准迁移 |
| wj04 | Novanest | novanest.one | starrats111/novanest | `js/data.js` | ❌ 无 | 标准迁移 |
| wj05 | Zontri | zontri.top | starrats111/zontri | `js/data.js` | ✅ 有 | ⚠️ 有产品模块 |
| wj06 | AllurahHub | allurahub.top | starrats111/allurahub | `js/data.js` | ❌ 无 | 标准迁移 |
| wj07 | VitaHaven | vitahaven.click | starrats111/vitahaven | `js/data.js` | ❌ 无 | 标准迁移 |
| wj08 | — | — | — | — | — | ❌ 已离职 |
| wj09 | BloomRoots | bloomroots.top | starrats111/bloomroots | `js/data.js` | ❌ 无 | 标准迁移 |
| wj10 | VitaSphere | vitasphere.top | starrats111/vitasphere | `js/data.js` | ❌ 无 | 标准迁移 |

---

## 一、迁移目的

将博客文章从**单一大文件**迁移到**每篇文章独立文件**的存储方式。

### 迁移前后对比

| 维度 | 迁移前 | 迁移后 |
|------|--------|--------|
| **存储方式** | 所有文章在一个文件 | 索引 + 每篇独立JSON |
| **首页加载** | 全部数据（可能500KB+） | 仅索引（~20KB） |
| **详情页** | 数据已在内存 | 按需加载（~5KB/篇） |
| **Git冲突** | 多人编辑会冲突 | 零冲突 |
| **容错性** | 一处错误全站崩 | 单篇独立 |
| **扩展性** | <50篇 | 1000+篇 |

### 迁移后目录结构

```
网站仓库/
├── js/
│   ├── data.js.backup       # 原始备份
│   ├── articles-index.js    # 【新】索引文件
│   └── articles/            # 【新】文章目录
│       ├── 1.json
│       ├── 2.json
│       └── ...
├── index.html
├── article.html
└── ...
```

---

## 二、通用迁移脚本

以下脚本适用于**所有网站**，只需修改配置部分。

### 2.1 迁移脚本 `migrate.js`

在网站仓库根目录创建此文件：

```javascript
/**
 * 博客数据迁移脚本
 * 
 * 功能：将 js/data.js 拆分为：
 * - js/articles-index.js (索引)
 * - js/articles/*.json (单篇文章)
 * 
 * 使用：node migrate.js
 */

const fs = require('fs');
const path = require('path');

// ================== 配置区域 ==================
// 【重要】根据不同网站修改此处
const CONFIG = {
  // 源文件路径（wj02需改为 'data.js'）
  sourceFile: 'js/data.js',
  
  // 输出路径
  outputIndexFile: 'js/articles-index.js',
  outputArticlesDir: 'js/articles',
  backupFile: 'js/data.js.backup',
  
  // 索引保留字段（列表页需要）
  indexFields: [
    'id', 'title', 'slug', 'category', 'categoryName',
    'date', 'image', 'excerpt', 'description', 'featured',
    'author', 'readTime'
  ],
  
  // 是否有产品模块（wj05设为true）
  hasProductModule: false
};
// ================== 配置结束 ==================

async function main() {
  console.log('🚀 开始迁移...\n');
  
  // 1. 检查源文件
  if (!fs.existsSync(CONFIG.sourceFile)) {
    console.error(`❌ 找不到 ${CONFIG.sourceFile}`);
    console.log('请确保在仓库根目录运行此脚本');
    process.exit(1);
  }
  
  // 2. 备份
  console.log('📦 备份原始文件...');
  fs.copyFileSync(CONFIG.sourceFile, CONFIG.backupFile);
  console.log(`   ✓ 已备份到 ${CONFIG.backupFile}\n`);
  
  // 3. 读取数据
  console.log('📖 读取源数据...');
  const sourceContent = fs.readFileSync(CONFIG.sourceFile, 'utf-8');
  const articles = parseArticles(sourceContent);
  console.log(`   ✓ 找到 ${articles.length} 篇文章\n`);
  
  if (articles.length === 0) {
    console.error('❌ 未找到文章数据，请检查data.js格式');
    process.exit(1);
  }
  
  // 4. 创建目录
  console.log('📁 创建输出目录...');
  if (!fs.existsSync(CONFIG.outputArticlesDir)) {
    fs.mkdirSync(CONFIG.outputArticlesDir, { recursive: true });
  }
  console.log(`   ✓ ${CONFIG.outputArticlesDir}\n`);
  
  // 5. 生成索引
  console.log('📝 生成索引文件...');
  const indexData = generateIndex(articles);
  const indexContent = `// 文章索引 - 生成于 ${new Date().toISOString()}
// 列表页使用此文件，详情页按需加载 articles/*.json

const articlesIndex = ${JSON.stringify(indexData, null, 2)};
`;
  fs.writeFileSync(CONFIG.outputIndexFile, indexContent, 'utf-8');
  console.log(`   ✓ ${CONFIG.outputIndexFile}\n`);
  
  // 6. 生成单篇文章
  console.log('📄 生成单篇文章文件...');
  let count = 0;
  for (const article of articles) {
    const filePath = path.join(CONFIG.outputArticlesDir, `${article.id}.json`);
    fs.writeFileSync(filePath, JSON.stringify(article, null, 2), 'utf-8');
    count++;
    console.log(`   ✓ ${article.id}.json - ${article.title.substring(0, 30)}...`);
  }
  
  // 7. 统计
  console.log('\n📊 迁移统计:');
  console.log(`   - 原文件: ${(fs.statSync(CONFIG.backupFile).size / 1024).toFixed(2)} KB`);
  console.log(`   - 索引文件: ${(fs.statSync(CONFIG.outputIndexFile).size / 1024).toFixed(2)} KB`);
  console.log(`   - 文章数量: ${count} 篇`);
  
  // 8. 完成
  console.log('\n✅ 迁移完成！\n');
  console.log('📋 下一步操作:');
  console.log('─────────────────────────────────────');
  console.log('1. 更新HTML中的script引用:');
  console.log('   将 <script src="js/data.js"> ');
  console.log('   改为 <script src="js/articles-index.js">');
  console.log('');
  console.log('2. 更新JS中的文章加载逻辑（见文档）');
  console.log('');
  console.log('3. 本地测试所有功能');
  console.log('');
  console.log('4. 推送到GitHub:');
  console.log('   git add .');
  console.log('   git commit -m "chore: 迁移文章存储格式"');
  console.log('   git push');
  console.log('─────────────────────────────────────\n');
}

// ================== 解析函数 ==================

function parseArticles(content) {
  try {
    // 尝试多种格式匹配
    let match = content.match(/(?:const|let|var)?\s*articles\s*=\s*(\[[\s\S]*\])\s*;?\s*$/);
    
    if (!match) {
      match = content.match(/(?:const|let|var)?\s*articles\s*=\s*(\[[\s\S]*?\]);/);
    }
    
    if (!match) {
      // 尝试匹配products数组（某些网站可能用不同名称）
      match = content.match(/(?:const|let|var)?\s*(?:articles|posts|data)\s*=\s*(\[[\s\S]*\])/);
    }
    
    if (!match) {
      throw new Error('找不到文章数组，请检查data.js格式');
    }
    
    const parseFunc = new Function(`return ${match[1]}`);
    const result = parseFunc();
    
    if (!Array.isArray(result)) {
      throw new Error('解析结果不是数组');
    }
    
    return result;
  } catch (error) {
    console.error('解析错误:', error.message);
    console.log('\n💡 提示: 请将data.js前50行发给wj07检查');
    process.exit(1);
  }
}

function generateIndex(articles) {
  return articles.map(article => {
    const entry = {};
    
    // 复制索引字段
    for (const field of CONFIG.indexFields) {
      if (article[field] !== undefined) {
        entry[field] = article[field];
      }
    }
    
    // 确保有摘要
    if (!entry.excerpt && !entry.description && article.content) {
      const text = article.content.replace(/<[^>]*>/g, '').trim();
      entry.excerpt = text.substring(0, 150) + '...';
    }
    
    // 标记产品模块
    entry.hasProducts = !!(article.products && article.products.length > 0);
    
    return entry;
  });
}

// 运行
main();
```

---

## 三、各网站迁移指南

### 3.1 wj01 EverydayHaven

**仓库**: https://github.com/starrats111/EverydayHaven

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/EverydayHaven.git
cd EverydayHaven

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.2 wj02 Quiblo ⚠️ 特殊处理

**仓库**: https://github.com/starrats111/Quiblo

**特殊说明**: data.js在根目录，需要先移动

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/Quiblo.git
cd Quiblo

# 2. 创建js目录并移动data.js
mkdir -p js
mv data.js js/data.js

# 3. 更新HTML中的引用
# 将所有 <script src="data.js"> 改为 <script src="js/data.js">

# 4. 创建 migrate.js（复制上方脚本，配置无需修改）

# 5. 运行迁移
node migrate.js

# 6. 更新HTML和JS（见第四章）

# 7. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.3 wj03 Kivanta

**仓库**: https://github.com/starrats111/kivanta

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/kivanta.git
cd kivanta

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.4 wj04 Novanest

**仓库**: https://github.com/starrats111/novanest

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/novanest.git
cd novanest

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.5 wj05 Zontri ⚠️ 有产品模块

**仓库**: https://github.com/starrats111/zontri

**特殊说明**: 有产品模块，需修改配置

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/zontri.git
cd zontri

# 2. 创建 migrate.js
# 【重要】修改配置中的 hasProductModule: true
```

**migrate.js 配置修改**:
```javascript
const CONFIG = {
  sourceFile: 'js/data.js',
  outputIndexFile: 'js/articles-index.js',
  outputArticlesDir: 'js/articles',
  backupFile: 'js/data.js.backup',
  indexFields: [
    'id', 'title', 'slug', 'category', 'categoryName',
    'date', 'image', 'excerpt', 'description', 'featured',
    'author', 'readTime'
  ],
  hasProductModule: true  // ← 改为 true
};
```

```bash
# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章，注意产品模块渲染）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.6 wj06 AllurahHub

**仓库**: https://github.com/starrats111/allurahub

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/allurahub.git
cd allurahub

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.7 wj07 VitaHaven

**仓库**: https://github.com/starrats111/vitahaven

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/vitahaven.git
cd vitahaven

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.8 wj09 BloomRoots

**仓库**: https://github.com/starrats111/bloomroots

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/bloomroots.git
cd bloomroots

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

### 3.9 wj10 VitaSphere

**仓库**: https://github.com/starrats111/vitasphere

```bash
# 1. 克隆仓库
git clone https://github.com/starrats111/vitasphere.git
cd vitasphere

# 2. 创建 migrate.js（复制上方脚本，配置无需修改）

# 3. 运行迁移
node migrate.js

# 4. 更新HTML和JS（见第四章）

# 5. 测试并推送
git add .
git commit -m "chore: 迁移文章存储格式"
git push
```

---

## 四、网站代码更新

### 4.1 HTML文件修改

在所有HTML文件中，找到：

```html
<script src="js/data.js"></script>
```

改为：

```html
<script src="js/articles-index.js"></script>
```

**需要修改的文件**（视网站结构而定）：
- `index.html`
- `article.html`
- `articles.html`
- `category.html`
- `about.html`
- `contact.html`

---

### 4.2 列表页JS（基本不变）

```javascript
// 原来使用 articles，现在使用 articlesIndex
// 只需改变量名

function renderArticleList() {
  // 使用 articlesIndex 代替原来的 articles
  articlesIndex.forEach(article => {
    // 渲染逻辑不变
  });
}
```

---

### 4.3 详情页JS（核心修改）

在 `js/main.js` 中添加以下代码：

```javascript
// ===============================================
// 文章详情页加载逻辑（迁移后必须添加）
// ===============================================

/**
 * 从URL获取文章ID
 */
function getArticleId() {
  const params = new URLSearchParams(window.location.search);
  // 支持 ?id=1 或 ?slug=xxx 两种格式
  return params.get('id') || params.get('slug');
}

/**
 * 从索引中查找文章
 */
function findInIndex(idOrSlug) {
  // 按ID查找
  let article = articlesIndex.find(a => String(a.id) === String(idOrSlug));
  // 按slug查找
  if (!article) {
    article = articlesIndex.find(a => a.slug === idOrSlug);
  }
  return article;
}

/**
 * 异步加载文章完整内容
 */
async function loadArticleContent(id) {
  try {
    const response = await fetch(`js/articles/${id}.json`);
    if (!response.ok) {
      throw new Error('文章加载失败');
    }
    return await response.json();
  } catch (error) {
    console.error('加载文章失败:', error);
    return null;
  }
}

/**
 * 渲染文章内容
 */
function renderArticleContent(article) {
  // 设置页面标题
  document.title = article.title + ' | ' + document.title.split('|').pop();
  
  // 渲染基本信息
  const titleEl = document.getElementById('article-title');
  const dateEl = document.getElementById('article-date');
  const categoryEl = document.getElementById('article-category');
  const imageEl = document.getElementById('article-image');
  const contentEl = document.getElementById('article-content');
  
  if (titleEl) titleEl.textContent = article.title;
  if (dateEl) dateEl.textContent = article.date;
  if (categoryEl) categoryEl.textContent = article.categoryName || article.category;
  if (imageEl) imageEl.src = article.image;
  if (contentEl) contentEl.innerHTML = article.content;
  
  // 渲染产品模块（如有）
  if (article.products && article.products.length > 0) {
    renderProducts(article.products);
  }
}

/**
 * 渲染产品模块
 */
function renderProducts(products) {
  const container = document.getElementById('products-container');
  if (!container) return;
  
  container.style.display = 'block';
  container.innerHTML = products.map(product => `
    <div class="product-card">
      <img src="${product.image}" alt="${product.name}">
      <h4>${product.name}</h4>
      <p>${product.description || ''}</p>
      <span class="price">${product.price || ''}</span>
    </div>
  `).join('');
}

/**
 * 显示错误页面
 */
function showError(message) {
  const container = document.getElementById('article-container') || document.body;
  container.innerHTML = `
    <div style="text-align: center; padding: 50px;">
      <h1>😕</h1>
      <h2>${message}</h2>
      <p><a href="index.html">返回首页</a></p>
    </div>
  `;
}

/**
 * 显示加载中
 */
function showLoading() {
  const container = document.getElementById('article-container');
  if (container) {
    container.innerHTML = `
      <div style="text-align: center; padding: 50px;">
        <p>加载中...</p>
      </div>
    `;
  }
}

/**
 * 初始化文章详情页
 */
async function initArticlePage() {
  showLoading();
  
  const idOrSlug = getArticleId();
  
  if (!idOrSlug) {
    showError('文章ID缺失');
    return;
  }
  
  // 从索引中查找
  const indexData = findInIndex(idOrSlug);
  
  if (!indexData) {
    showError('文章不存在');
    return;
  }
  
  // 加载完整内容
  const article = await loadArticleContent(indexData.id);
  
  if (!article) {
    showError('文章加载失败');
    return;
  }
  
  // 渲染
  renderArticleContent(article);
}

// 检测是否是详情页，自动初始化
if (window.location.pathname.includes('article.html') || 
    window.location.pathname.includes('post.html')) {
  document.addEventListener('DOMContentLoaded', initArticlePage);
}
```

---

## 五、验证清单

每个网站迁移完成后，请逐项验证：

| 检查项 | 状态 |
|--------|------|
| 首页文章列表显示正常 | ☐ |
| 点击文章能跳转到详情页 | ☐ |
| 详情页标题正确显示 | ☐ |
| 详情页内容完整显示 | ☐ |
| 详情页图片正常加载 | ☐ |
| 产品模块正常显示（如有） | ☐ |
| 分类筛选功能正常 | ☐ |
| 搜索功能正常（如有） | ☐ |
| 分页功能正常（如有） | ☐ |
| 浏览器控制台无错误 | ☐ |
| 移动端显示正常 | ☐ |

---

## 六、回滚方案

如果迁移出现问题，可以快速回滚：

```bash
# 1. 恢复原始data.js
cp js/data.js.backup js/data.js

# 2. 恢复HTML中的引用（手动改回 js/data.js）

# 3. 删除迁移生成的文件
rm -rf js/articles/
rm js/articles-index.js

# 4. 撤销JS修改（如已修改main.js）

# 5. 推送回滚
git add .
git commit -m "revert: 回滚文章迁移"
git push
```

---

## 七、常见问题

### Q1: 运行脚本报错"找不到文章数组"

**原因**: data.js格式与脚本预期不符  
**解决**: 将data.js前50行发给wj07，调整脚本

### Q2: 详情页显示空白

**原因**: JS加载逻辑未添加  
**解决**: 确认已按第四章4.3添加代码到main.js

### Q3: 文章链接跳转404

**原因**: URL参数格式问题  
**解决**: 确保文章链接格式为 `article.html?id=1`

### Q4: 首页文章列表不显示

**原因**: 变量名未更新  
**解决**: 将 `articles` 改为 `articlesIndex`

### Q5: 产品模块不显示

**原因**: products-container元素不存在  
**解决**: 在article.html中添加 `<div id="products-container"></div>`

---

## 八、迁移状态追踪

| 网站 | 负责人 | 状态 | 完成日期 | 备注 |
|------|--------|------|----------|------|
| EverydayHaven | wj01 | 🟡 待迁移 | | |
| Quiblo | wj02 | 🟡 待迁移 | | 需先移动data.js |
| Kivanta | wj03 | 🟡 待迁移 | | |
| Novanest | wj04 | 🟡 待迁移 | | |
| Zontri | wj05 | 🟡 待迁移 | | 有产品模块 |
| AllurahHub | wj06 | 🟡 待迁移 | | |
| VitaHaven | wj07 | 🟡 待迁移 | | |
| BloomRoots | wj09 | 🟡 待迁移 | | |
| VitaSphere | wj10 | 🟡 待迁移 | | |

**状态说明**:
- 🟡 待迁移
- 🔵 迁移中
- 🟢 已完成
- 🔴 迁移失败

---

## 九、联系方式

如有问题，请联系：**wj07**

---

**文档版本**: v1.0  
**创建日期**: 2026-02-13  
**最后更新**: 2026-02-13

